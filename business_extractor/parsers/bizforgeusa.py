"""
parsers/bizforgeusa.py
Parser for bizforgeusa.com business directory profile pages.

Same underlying directory template as americasmallbiz.com (see that
parser's docstring for the general approach: JSON-LD LocalBusiness block
first, HTML fallback selectors second, streetAddress always split via
_split_blinx_address() since addressLocality/addressRegion/postalCode
are hardcoded "N/A" in the schema on this template).

bizforgeusa.com has one additional quirk not seen on americasmallbiz.com:
the "Visit Website" link and the JSON-LD sameAs both concatenate the
business's real URL directly against a trailing label with no separator,
e.g. "https://wrightway.com" + "About us:" -> "https://wrightway.comAboutus:".
_clean_glued_website_url() below detects and strips that glued suffix by
finding the end of the domain's TLD and checking whether what follows is
a valid URL separator (/, ?, #, :) or end-of-string; if not, everything
after the TLD is discarded as the glued artifact. A legitimate path
("https://example.com/services") is left untouched since '/' is a valid
separator. Unrecognized TLDs fall through unchanged rather than risk
mangling a URL this parser doesn't understand.
"""
...
def _clean_glued_website_url(url):
    """Strip a label glued directly onto a URL with no separator (this
    site's "Visit Website" bug). Returns the URL unchanged if it already
    ends cleanly, has a real path/query/fragment after the domain, or if
    the TLD isn't recognized."""
    if not url:
        return ""
    url = url.strip()
    ...
    # observed case) is the label-concatenation artifact -- truncate.
    return url[:end]


def _extract_bizforgeusa_jsonld(soup):
...
def parse_bizforgeusa(url, html):
    soup = BeautifulSoup(html, "html.parser")
    business = empty_business()

    jsonld = _extract_bizforgeusa_jsonld(soup)
    ...
    # telephone (which drops formatting, e.g. "941379-8669").
