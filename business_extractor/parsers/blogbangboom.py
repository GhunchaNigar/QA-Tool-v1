"""
Parser for blogbangboom.com business profile pages.

Example URL:
  https://www.blogbangboom.com/business-services/wrightway-emergency-services

Same directory-network theme as biz411.org / bizbangboom.com / bizbuildboom.com
/ bizlinkbuilder.com (see parsers/bizlinkbuilder.py -- identical OptimizeCDN
asset paths, identical single-column (col-lg-12, no sidebar) layout, and
identical "table-view-group" / "col-sm-4" + "col-sm-8" row markup). This
parser is effectively a copy of parsers/bizlinkbuilder.py; the sampled
markup for this domain matches it field-for-field:

  - Single-column layout (col-lg-12), same as bizlinkbuilder.com -- no
    sidebar "Share This Page" module to accidentally scrape as the
    business's own Social Media Links.
  - Phone number is plain text, not hidden behind a "See Phone Number"
    reveal link: both the "Phone Number" contact row
    (`.table-display-phone .col-sm-8`) and the profile-header phone
    display (`.author-phone`) already contain the number in the raw HTML.
  - Address is comma-separated in the standard "Street, City, State Zip"
    shape here (e.g. "300 Triple Diamond Blvd, Nokomis, FL 34275"),
    which _split_blinx_address() handles directly (3 comma-parts ->
    street / city / "state zip").
  - No dedicated Hours row was present on the sampled page (the "Company
    Details" section was empty); extraction falls back to a generic
    ".table-view-group" scan for a row labelled "hour".
  - Description comes from the single Froala about-me paragraph, with a
    trailing empty `<p><br></p>` (leftover pasted-Word-doc markup)
    dropped automatically since it has no meaningful text.
"""
...
def _phone_value(soup, paragraphs):
    # Primary: the dedicated "Phone Number" contact row -- the number here
    # is plain text, not hidden behind a "See Phone Number" reveal link.
...
def _social_links(soup):
    ...
    Contact Information block only. This layout has no sidebar at all, but
    the scoping is kept anyway as a defensive measure in case another
    listing on this domain does render one."""
    links = {}
...
def parse_blogbangboom(url, html):
